...
export default function DoctorsDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { homeDataById, loading, error, loadFromStorage } = useHomeStore()
  console.log(service);

  useEffect(() => {
    loadFromStorage()
    const fetchService = async () => {
      const data = await homeDataById(id);
      setService(data[0]);
    };
    fetchService();
  }, [id, loadFromStorage]);
    ...
    <div className="">
      ...
      {loading ? <div className='h-80 w-full flex justify-center items-center animate-spin'><LuLoaderCircle size={25} /></div> :
        <div className="overflow-hidden max-w-3xl mx-auto p-4 pt-30">
          ...
          />
          <div className="p-6">
            <h1 className="text-2xl font-bold mb-2">{service?.user_name}</h1>
